...
#
# Usage python3 observable_to_vanilla.py [download_dir_path] [output_dir_path]
#
...
def load_info(dir_path):
    ...
    for f_script in js_func_scripts:
        ...
        if chart_func_mark in f_script:
            chart_func_script = f_script.split(chart_func_mark)[1]
            var_name_chart_func = chart_func_script.split(',')[0]
            var_name_data = chart_func_script.split(',')[1]

            attr_desc = chart_func_script.split(f'{var_name_chart_func}(')[1]
            attr_desc = attr_desc[:attr_desc.rfind('})') + 1]

            if 'width,\n' in attr_desc:
                attr_desc = attr_desc.replace('width,\n', f'width: {width},\n')
    ...
    for f_script in js_func_scripts:
        chart_func_mark = f'{var_name_chart_func}(data,'
        if chart_func_mark in f_script:
            chart_func_script = f'function {chart_func_mark}{f_script.split(chart_func_mark)[1]}'
